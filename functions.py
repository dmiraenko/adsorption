...
class Sips(AdsorptionFunction):
    ...
    def __init__(self, sat_surface_conc, b, n : Quantity):
        ...
        self.surface_conc_func = lambda p : self.sat_surface_conc * (q := (self.b * p) ** self.ninv) / (1 + q)
        self.henry_law_const = 0 
        self.red_spr_pressure_func = lambda p0 : self.n * self.sat_surface_conc * log(1 + (self.b * p0) ** self.ninv)
